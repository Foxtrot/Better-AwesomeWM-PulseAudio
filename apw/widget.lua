...
function pulseWidget.setColor(mute)
	if mute then
		...
		pulseText:set_markup('<span color="#cc6666"> '..math.ceil(p.Volume*100)..'% </span>')
	else
		pulseBar:set_color(color)
		pulseBar:set_background_color(color_bg)
		pulseText:set_font("Tewi 9")
    		pulseText:set_markup('<span color="#FFFFFF"> '..math.ceil(p.Volume*100)..'% </span>')
	end
end
...
local function _update(mute)
  ...
  --  if show_text then
    --    pulseText:set_markup('<span color="#FF0000">'..math.ceil(p.Volume*100)..'%</span>')
   -- end
end
